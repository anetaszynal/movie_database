...
export const Pagination:FC<{page?: number, totalPages?: number}> = ({ page, totalPages }) => {
  ...
  return (
    <Wrapper>
      ...
      <ChangePageButton onClick = {page ? () => goToPage(page - 1) : () => {}} disabled = {isFirstPage}>
        <Arrow disabled = {isFirstPage} back/>
      ...
      <ChangePageButton onClick = {page ? () => goToPage(page + 1): () => {}} disabled = {isLastPage}>
        <ButtonTitle>Next</ButtonTitle>
        <Arrow disabled = {isLastPage}/>
      </ChangePageButton>
      <ChangePageButton onClick = {totalPages ? () => goToPage(totalPages) : () => {}} disabled = {isLastPage}>
